...
function cutewp_register_theme_customizer( $wp_customize ) {

    if(method_exists('WP_Customize_Manager', 'add_panel')):
    $wp_customize->add_panel('cutewp_main_options_panel', array( 'title' => esc_html__('Theme Options', 'cutewp'), 'priority' => 10, ));
    endif;
    
    $wp_customize->get_section( 'title_tagline' )->panel = 'cutewp_main_options_panel';
    $wp_customize->get_section( 'title_tagline' )->priority = 20;
    $wp_customize->get_section( 'colors' )->panel = 'cutewp_main_options_panel';
    ...
    cutewp_getting_started($wp_customize);
    cutewp_header_options($wp_customize);
    cutewp_post_options($wp_customize);
    cutewp_social_profiles($wp_customize);
    cutewp_footer_options($wp_customize);
    cutewp_other_options($wp_customize);
    cutewp_recomm_plugin_options($wp_customize);
    cutewp_upgrade_to_pro($wp_customize);

}

add_action( 'customize_register', 'cutewp_register_theme_customizer' );

function cutewp_customizer_js_scripts() {
    wp_enqueue_script('cutewp-theme-customizer-js', get_template_directory_uri() . '/inc/admin/js/customizer.js', array( 'jquery', 'customize-preview' ), NULL, true);
}
add_action( 'customize_preview_init', 'cutewp_customizer_js_scripts' );